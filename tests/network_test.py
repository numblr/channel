...
class NetworkDefinitionTestCase(TestCase):
    ...
    def test_add_module(self):
        ...
        self.assertEqual(self.definition.available_module_ids(), ["test"])

    def test_add_multiple_modules(self):
        self.__add_module_one_to_four()
        
        self.assertEqual(self.definition.available_module_ids(), ["one", "two", "three", "four"])
    ...
    def test_invalid_channel_type(self):
        self.assertRaisesRegex(UndefinedNameError, "other", self.definition.add_module, "test", "other")

    def test_conflicting_module_name(self):
        self.definition.add_module("test", "typeone")

        self.assertRaisesRegex(NameConflictError, "test", self.definition.add_module, "test", "typetwo")

    def test_invalid_module_order(self):
        self.__add_module_one_to_four()
        
        self.assertRaisesRegex(IllegalOrderError, "\"two\" must have been defined before \"one\"", self.definition.add_connection, "two", "one")
        
    def test_invalid_module_name_in_connection(self):
        self.__add_module_one_to_four()
        
        self.assertRaisesRegex(UndefinedNameError, "five", self.definition.add_connection, "one", "five")
        self.assertRaisesRegex(UndefinedNameError, "five", self.definition.add_connection, "five", "two")
        
    def test_duplicate_connection(self):
        self.__add_module_one_to_four()
        self.definition.add_connection("one", "two")
        
        self.assertRaisesRegex(NameConflictError, "\"one\" is already connected to \"two\"", self.definition.add_connection, "one", "two")
...
class NetworkFactoryTestCase(TestCase):
    ...
    def test_available_types(self):
        self.assertCountEqual(self.factory.available_channel_types(), ("sum", "reverse", "delay"))
        
    def test_create(self):
        test_network = self.factory.create(TEST_DEFINITION)()
        output = test_network.send("test")
        
        self.assertEqual(output, "tset")
        
    def test_define(self):
        self.factory.define_channel_type("test", TEST_DEFINITION)
        
        self.assertCountEqual(self.factory.available_channel_types(), ("sum", "reverse", "delay", "test"))
    ...
    def test_create_independent(self):
        ...
        self.assertEqual(output_1, DELAY_INITIAL)
        self.assertEqual(output_2, DELAY_INITIAL)
    ...
    def test_create_and_start_multiple_times(self):
        ...
        for i in range(5):
            ...
            self.assertEqual(tuple(islice(outputs, 3)), (DELAY_INITIAL, "test", ""), "Failed at {0}".format(i))
    ...
    def test_define_with_name_conflict(self):
        self.assertRaisesRegex(NameConflictError, "sum", self.factory.define_channel_type, "sum", TEST_DEFINITION)
...
class NetworkTestCase(TestCase, ChannelTestCase, NoInputTestCase):
    ...
    def test_empty(self):
        ...
        self.assertEqual(empty_network.send("test"), None)
